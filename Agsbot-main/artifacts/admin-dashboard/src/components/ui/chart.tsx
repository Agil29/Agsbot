...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const [item] = payload
      const key = `${labelKey || String(item?.["dataKey"] ?? "") || String(item?.["name"] ?? "") || "value"}`
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload
            ...
            .map((item, index) => {
              const key = `${nameKey || String(item["name"] ?? "") || String(item["dataKey"] ?? "") || "value"}`
              const itemConfig = getPayloadConfigFromPayload(config, item, key)
              const indicatorColor = color || String((item["payload"] as Record<string, unknown>)?.["fill"] ?? "") || String(item["color"] ?? "")
